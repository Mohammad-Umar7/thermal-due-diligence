...
import { Caveats, Chain, Clause, FieldBlock, Footer, HowCalculated, Masthead } from "@/components/Document";
...
export default async function ReportPage({ params }: PageProps<"/report/[id]">) {
  ...
  const { city, parcel } = found;
  const r = buildReport(city, parcel);
    ...
    <>
      ...
      <main className="mx-auto w-full max-w-5xl flex-1 px-5 sm:px-8">
          ...
          <div className="mt-6">
            <FieldBlock
              fields={[
                ...
                { label: "Reference station", value: city.station.name.replace(/, [A-Z]{2}$/, "") },
                {
                  label: "Record window",
                  value: (
                    <span className="figure text-[12.5px]">
        ...
        <Clause
          ...
          lede={`The published design condition for this site is ${fmt(r.gap.standardC)} °C, measured at ${city.station.name.replace(/, [A-Z]{2}$/, "")} over ${city.noaa.historicWindow}. Two independent corrections apply at this address.`}
        >
          <GapDiagram
          ...
          <p className="mt-7 max-w-2xl border-t border-rule pt-5 text-[15px] leading-relaxed">
            {hot ? (
              <>
                ...
                undersized by{" "}
                <strong className="figure font-semibold">{fmt(r.gap.combinedC)} °C</strong> on
                the design day.
              </>
            ) : (
              <>
          ...
          <HowCalculated summary="How the published standard was computed">
            ...
            <p className="mt-4 text-[12.5px] leading-relaxed text-ink-muted">
              ASHRAE publishes design conditions in licensed tables. None is used
              here. We apply the published <em>method</em> — the temperature
              exceeded 0.4% of hours in an average year — to public-domain NOAA
              observations. On the 1991–2020 window this reproduces the figure the
              industry designs to for Phoenix (43.90 °C), which is how we know the
              method is right.
            </p>
          </HowCalculated>
          ...
          <HowCalculated summary={`How the spatial component (${signed(r.gap.spatialC)} °C) was computed`}>
            ...
            <p className="mt-4 text-[12.5px] leading-relaxed text-ink-muted">
              Both temperatures come from the same FortyGuard request, over the same
              period, at the same granularity — so the difference reflects location
              rather than method. FortyGuard&rsquo;s absolute level is never subtracted
              from NOAA&rsquo;s; only the offset between two FortyGuard points is used.
            </p>
          </HowCalculated>
        ...
        {/* ---- §3 the station's operational record ---- */}
        <Clause
          n="3"
        ...
        >
          ...
          <HowCalculated summary="How cooling degree days were computed">
            ...
            />
            <p className="mt-4 text-[12.5px] leading-relaxed text-ink-muted">
              We stop at degree days. Converting these to an energy cost would need
              a published tariff and a stated system efficiency, and the second is a
              property of a building we have not seen. A figure with a guessed link
              in its chain is worse than no figure.
            </p>
          </HowCalculated>
        </Clause>

        {/* ---- §4 validation, shown rather than hidden ---- */}
        <Clause
          n="4"
          title="Validation"
          lede="The check that would break this method, run and reported."
        >
          <p className="max-w-2xl text-[14px] leading-relaxed">
            FortyGuard is an interpolated model and smooths temperature extremes.
            Comparing its peak against the station&rsquo;s observed peak at the{" "}
            <em>same point</em> measures that smoothing — and shows why its absolute
            level is never used here.
          </p>
          <div className="mt-5 overflow-x-auto">
            <table className="w-full min-w-[420px] border-collapse text-[13.5px]">
              <caption className="label pb-2 text-left">
                {city.station.name.replace(/, [A-Z]{2}$/, "")}, July 2024, same coordinates
              </caption>
              <tbody>
                <tr className="border-b border-rule">
                  <td className="py-2 text-ink-muted">NOAA observed maximum</td>
                  <td className="figure py-2 text-right">{city.validation.noaaJuly2024MaxC?.toFixed(2)} °C</td>
                </tr>
                <tr className="border-b border-rule">
                  <td className="py-2 text-ink-muted">FortyGuard modelled maximum</td>
                  <td className="figure py-2 text-right">{city.validation.fortyguardJuly2024MaxC.toFixed(2)} °C</td>
                </tr>
          ...
          <p className="mt-4 max-w-2xl text-[12.5px] leading-relaxed text-ink-muted">
            This difference is a property of the model, not of any location. It is
            why the absolute level always comes from NOAA and FortyGuard supplies
            only the offset between two points measured the same way.
          </p>
        </Clause>
